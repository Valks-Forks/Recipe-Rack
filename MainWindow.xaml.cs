...
public partial class MainWindow : Window
{
    ...
    private void add_New_Recipe_Click(object sender, RoutedEventArgs e)
    {
        ...
        EnterNewRecipeMenu enterNewRecipeMenu = new()
        {
            Owner = this,
    ...
    private void EditRecipe_Button_Click(object sender, RoutedEventArgs e)
    {
        if (!(SelectRecipes_ListBox.SelectedItem == null))
        {
            ...
            EnterNewRecipeMenu enterEditRecipeMenu = new()
            {
                Owner = this,
                WindowStartupLocation = WindowStartupLocation.Manual,
                Top = this.Top + 150,
                Left = this.Left + 300,
                IsThisAnEdit = true,
                OldRecipeName = Card_RecipeName_Label.Content.ToString().Trim()
            };
    ...
    private void Show_UIControls(bool show)
    {
        if (show == true)
        {
            ...
            var recipes = SqliteDataAccess.LoadAllRecipes();
            GetStarted_Label.Visibility = recipes.Count > 0 ? Visibility.Hidden : Visibility.Visible;
        }
        else
        {
            ...
            //Check if the user has any recipes already if they do then dont show the GetStarted label if they dont display it
            var recipes = SqliteDataAccess.LoadAllRecipes();
            GetStarted_Label.Visibility = recipes.Count == 0 ? Visibility.Visible : Visibility.Hidden;
        }
    }
}
