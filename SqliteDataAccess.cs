    ...
    class SqliteDataAccess
    {
        ...
        public static List<Recipe> LoadAllRecipes()
        {
            ...
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                ...
                while (reader.Read())
                {
                    var thisRecipe = new Recipe(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetString(3), reader.GetInt32(4), reader.GetInt32(5));
        ...
        public static void DeleteRecipe(string recipeName)
        {
            using (IDbConnection cnn = new SQLiteConnection(LoadConnectionString()))
            {
                ...
                CMD.Connection = (SQLiteConnection)cnn;
                CMD.CommandType = CommandType.Text;
                CMD.CommandText = String.Format("DELETE FROM Recipe WHERE RecipeName='{0}'", recipeName);
                CMD.Connection.Open();
                CMD.ExecuteNonQuery();
                CMD.Connection.Close();
            }
